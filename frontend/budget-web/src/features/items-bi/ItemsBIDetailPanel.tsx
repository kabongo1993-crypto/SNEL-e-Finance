...
import { formatDateFr, libelleParent, libelleStatut } from './itemBIUtils';

interface ItemsBIDetailPanelProps {
  selected: ItemBI | null;
}
...
export function ItemsBIDetailPanel({ selected }: ItemsBIDetailPanelProps) {
  if (!selected) {
    return (
      <Paper sx={{ height: '100%', minHeight: 280, display: 'grid', placeItems: 'center', p: 2 }}>
        <Typography color="text.secondary" variant="body2" sx={{ textAlign: 'center' }}>
          Sélectionnez un item BI pour afficher ses détails.
        </Typography>
      </Paper>
    );
  }
  ...
  return (
    <Paper sx={{ height: '100%', minHeight: 280, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
      ...
      <Box sx={{ flex: 1, overflow: 'auto', px: 1.75, py: 1 }}>
        <InfoRow icon={<CodeOutlinedIcon fontSize="small" />} label="Code" value={selected.codeItem} />
        <InfoRow icon={<CodeOutlinedIcon fontSize="small" />} label="Libellé" value={selected.libelle} />
        <InfoRow icon={<AccountTreeOutlinedIcon fontSize="small" />} label="Parent" value={libelleParent(selected)} />
        <InfoRow icon={<AccountTreeOutlinedIcon fontSize="small" />} label="Niveau" value={String(selected.niveau)} />
        <InfoRow
          icon={<CategoryOutlinedIcon fontSize="small" />}
          label="Catégorie"
          value={selected.categorie?.trim() ? selected.categorie : '—'}
        />
