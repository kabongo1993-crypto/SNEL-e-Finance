...
[ApiController]
[Route("api/v1/rapports/previsions-ae")]
public class RapportsPrevisionsAeController : ControllerBase
{
    private readonly IRapportPrevisionAeService _service;

    public RapportsPrevisionsAeController(IRapportPrevisionAeService service)
    {
        _service = service;
    }
        ...
        CancellationToken cancellationToken)
    {
        try
        {
            var dto = await _service.GetAsync(
                new RapportAeQuery(idVersion, niveau, mois, idEntite, idDepartementStructure, idDivision, idUB, statutConsultation),
                cancellationToken);
            return Ok(dto);
        }
        ...
        CancellationToken cancellationToken)
    {
        try
        {
            var query = new RapportAeQuery(idVersion, niveau, mois, idEntite, idDepartementStructure, idDivision, idUB, statutConsultation);
